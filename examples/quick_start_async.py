...
使用方法:
1. 运行: python examples/quick_start_async.py
2. 测试 OpenAI 协议:
...
# =============================================================================
# 简单的 Agent 实现（带工具调用，异步版本）
# =============================================================================


class AsyncSimpleAgent:
    """简单的 Agent 实现，支持工具调用和生命周期钩子（异步版本）"""
    ...
    async def run(
        ...
        if needs_time:
            ...
            await asyncio.sleep(0)  # 让出控制权，确保流式
        ...
        for char in response:
            await asyncio.sleep(0.02)  # 小延迟确保流式效果可见
            yield char


# 创建 Agent 实例
agent = AsyncSimpleAgent(tools=TOOLS)
...
async def invoke_agent(request: AgentRequest) -> AsyncIterator:
    """Agent 调用处理函数（异步版本）
    ...
    try:
        ...
        # 运行 Agent（异步）
        async for chunk in agent.run(user_message, hooks):
            yield chunk
            await asyncio.sleep(0)  # 让出控制权，确保每个 chunk 立即发送
