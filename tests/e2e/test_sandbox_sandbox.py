...
class TestSandbox:
    ...
    def test_create_code_interpreter_sandbox(
        self, template_name_code_interpreter: str
    ):
        """测试创建 Code Interpreter 类型的 Sandbox"""
        # 先创建模板
    ...
    def test_get_sandbox(self, template_name_code_interpreter: str):
        """测试获取 Sandbox"""
        # 先创建模板
        template = Template.create(
            TemplateInput(
        ...
        try:
            # 创建 Sandbox
            created_sandbox = Sandbox.create(
                template_type=TemplateType.CODE_INTERPRETER,
                template_name=template_name_code_interpreter,
                sandbox_idle_timeout_seconds=600,
            )
            ...
            # 通过实例方法获取 Sandbox
            fetched_sandbox = created_sandbox.get()
    ...
    def test_delete_sandbox(self, template_name_code_interpreter: str):
        ...
        try:
            ...
            # 删除 Sandbox
            deleted_sandbox = Sandbox.delete_by_id(
                sandbox_id=sandbox.sandbox_id
            )
            assert deleted_sandbox is not None
    ...
    def test_delete_sandbox_via_instance_method(
        self, template_name_code_interpreter: str
    ):
        """测试通过实例方法删除 Sandbox"""
        # 先创建模板
    ...
    def test_sandbox_lifecycle(self, template_name_code_interpreter: str):
        ...
        try:
            ...
            # 4. 获取 Sandbox
            fetched_sandbox = connected_sandbox.get()
            assert fetched_sandbox.sandbox_id == sandbox.sandbox_id
    ...
    def test_sandbox_with_different_template_types(
        self, template_name_code_interpreter: str, template_name_browser: str
    ):
        """测试不同类型的 Template 创建 Sandbox"""
        # 创建 Code Interpreter 模板
        ci_template = Template.create(
            TemplateInput(
                template_name=template_name_code_interpreter,
                template_type=TemplateType.CODE_INTERPRETER,
                cpu=2.0,
        ...
        # 创建 Browser 模板
        browser_template = Template.create(
            TemplateInput(
                template_name=template_name_browser,
                template_type=TemplateType.BROWSER,
                cpu=2.0,
        ...
        try:
            ...
            # 验证可以分别连接
            connected_ci = Sandbox.connect(
                sandbox_id=ci_sandbox.sandbox_id,
                template_type=TemplateType.CODE_INTERPRETER,
            )
            assert connected_ci.sandbox_id == ci_sandbox.sandbox_id

            connected_browser = Sandbox.connect(
                sandbox_id=browser_sandbox.sandbox_id,
                template_type=TemplateType.BROWSER,
            )
            assert connected_browser.sandbox_id == browser_sandbox.sandbox_id
    ...
    def test_connect_with_wrong_template_type(
        self, template_name_code_interpreter: str
    ):
        """测试用错误的 template_type 连接 Sandbox 会抛出异常"""
        # 创建 Code Interpreter 模板
    ...
    def test_create_sandbox_without_template(self):
        """测试没有提供 template_name 时创建 Sandbox 会抛出异常"""
        with pytest.raises(ValueError, match="template_name is required"):
            Sandbox.create(
                template_type=TemplateType.CODE_INTERPRETER,
    ...
    def test_connect_without_template_type(
        self, template_name_code_interpreter: str
    ):
        """测试不提供 template_type 参数也能连接 Sandbox"""
        # 创建模板
