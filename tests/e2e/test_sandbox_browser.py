...
class TestSandboxBrowser:
    ...
    def test_browser_health_check(self, browser_template: Template):
        """测试浏览器健康检查"""
        sb = Sandbox.create(
            template_type=TemplateType.BROWSER,
            template_name=browser_template.template_name,
    ...
    def test_browser_vnc_cdp_urls(self, browser_template: Template):
        """测试获取 VNC 和 CDP 连接 URL"""
        sb = Sandbox.create(
            template_type=TemplateType.BROWSER,
            template_name=browser_template.template_name,
    ...
    def test_browser_recordings(self, browser_template: Template):
        """测试浏览器录制功能"""
        sb = Sandbox.create(
            template_type=TemplateType.BROWSER,
            template_name=browser_template.template_name,
        ...
        try:
            ...
            with sb.sync_playwright(record=True) as playwright:
                playwright.goto("https://www.baidu.com")
                time.sleep(3)  # 给录制足够的时间
                playwright.screenshot(path=screenshot_path)
                time.sleep(2)  # 再等待一些时间确保录制
    ...
    def test_browser_lifecycle(self, browser_template: Template):
        """测试浏览器 Sandbox 完整生命周期"""
        # 1. 创建
        sb = Sandbox.create(
            template_type=TemplateType.BROWSER,
    ...
    def test_browser_connect(self, browser_template: Template):
        """测试连接到已存在的浏览器 Sandbox"""
        # 创建 Sandbox
        sb1 = Sandbox.create(
            template_type=TemplateType.BROWSER,
        ...
        try:
            ...
            retry_count = 0
            while retry_count < max_retries:
                health_status = sb1.check_health()
                if health_status.get("status") == "ok":
                    break
                time.sleep(2)
                retry_count += 1
            ...
            # 使用连接的 Sandbox 进行操作
            assert isinstance(sb2, BrowserSandbox)
            with sb2.sync_playwright() as playwright:
                playwright.goto("https://www.baidu.com")
                title = playwright.title()
                assert "百度" in title
        finally:
