...
class TestSandboxCodeInterpreter:
    ...
    def test_context_execute_shell(self, sandbox):
        """测试执行 Shell 命令（通过 process.cmd）"""
    ...
    def test_filesystem_list(self, sandbox):
        """测试列举目录"""
        result = sandbox.file_system.list(path="/")
        assert result is not None
        assert isinstance(result, (list, dict))
    ...
    def test_filesystem_stat(self, sandbox):
        """测试获取文件/目录状态"""
        # 创建测试目录
        test_dir = f"/test-stat-{int(time.time())}"
        sandbox.file_system.mkdir(path=test_dir)

        # 获取状态
        result = sandbox.file_system.stat(path=test_dir)
        assert result is not None
        assert isinstance(result, dict)
    ...
    def test_filesystem_remove(self, sandbox):
        """测试删除文件/目录"""
        # 创建测试目录
        test_dir = f"/test-remove-{int(time.time())}"
        sandbox.file_system.mkdir(path=test_dir)
        ...
        # 删除目录
        result = sandbox.file_system.remove(path=test_dir)
        assert result is not None
    ...
    def test_file_overwrite(self, sandbox):
        """测试覆盖写入文件"""
        test_path = f"/test-overwrite-{int(time.time())}.txt"
        ...
        # 覆盖写入
        new_content = "new content"
        sandbox.file.write(path=test_path, content=new_content)
    ...
    def test_file_nested_directory(self, sandbox):
        """测试在嵌套目录中创建文件"""
        # 创建嵌套目录
        nested_dir = f"/test-nested-{int(time.time())}/subdir"
        sandbox.file_system.mkdir(path=nested_dir)
    ...
    def test_process_cmd(self, sandbox):
        """测试执行命令"""
        result = sandbox.process.cmd(command="ls", cwd="/")
        assert result is not None
        assert isinstance(result, dict)
        # 验证命令执行成功
        inner_result = result.get("result", {})
    ...
    def test_process_get(self, sandbox):
        ...
        if isinstance(processes, list) and len(processes) > 0:
            first_pid = processes[0].get("pid", "1")
            result = sandbox.process.get(pid=str(first_pid))
            assert result is not None
        else:
            # 尝试获取 PID 1 的信息
    ...
    def test_process_cmd_multiple_commands(self, sandbox):
        """测试执行多个命令"""
        # 执行多个命令
        commands = [
            "echo 'command 1'",
        ...
        for cmd in commands:
            result = sandbox.process.cmd(command=cmd, cwd="/")
            assert result is not None
    ...
    def test_process_cmd_different_cwd(self, sandbox):
        """测试在不同工作目录执行命令"""
        # 创建测试目录
        test_dir = f"/test-cwd-{int(time.time())}"
        sandbox.file_system.mkdir(path=test_dir)
        ...
        # 在测试目录中执行命令
        result = sandbox.process.cmd(command="pwd", cwd=test_dir)
        assert result is not None
...
""".format(workflow_dir.replace("/workflow-", ""))
    ...
    def test_code_interpreter_workflow(self, sandbox):
        """测试完整的 Code Interpreter 工作流"""
        workflow_dir = f"/workflow-{int(time.time())}"
...
""".format(workflow_dir.replace("/workflow-", ""))
        ...
        sandbox.file.write(path=script_path, content=script_content)
        ...
        # 4. 验证生成的数据文件
        data_path = f"{workflow_dir}/data.txt"
        read_result = sandbox.file.read(path=data_path)
        content = (
            read_result.get("content", read_result)
            if isinstance(read_result, dict)
            else read_result
    ...
    def test_concurrent_operations(self, sandbox):
        """测试并发操作"""
        base_dir = f"/concurrent-{int(time.time())}"
    ...
    def test_large_file_operations(self, sandbox):
        ...
        # 写入大文件
        sandbox.file.write(path=test_path, content=large_content)
    ...
    def test_error_handling_nonexistent_file(self, sandbox):
        """测试读取不存在的文件的错误处理"""
        nonexistent_path = "/nonexistent-file-xyz.txt"

        # 尝试读取不存在的文件应该引发异常或返回错误
        try:
            result = sandbox.file.read(path=nonexistent_path)
            # 如果没有抛出异常，检查返回值是否表明错误
            if isinstance(result, dict):
                assert "error" in result or result.get("success") is False
        except Exception:
    ...
    def test_sandbox_context_manager(self, template):
        """测试使用上下文管理器创建 Sandbox"""
        with Sandbox.create(
            template_type=TemplateType.CODE_INTERPRETER,
            template_name=template.template_name,
    ...
    def test_sandbox_health_check(self, sandbox):
        """测试健康检查"""
