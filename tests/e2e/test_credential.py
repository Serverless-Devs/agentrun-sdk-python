...
class TestCredential:
    ...
    def test_credential_lifecycle(self, credential_name: str):
        """测试凭证的完整生命周期"""
        client = CredentialClient()
        ...
        cred2 = client.get(credential_name=credential_name)
        ...
        # 列举 credentials
        credentials = Credential.list_all()
        assert len(credentials) > 0
        matched_cred = 0
        for c in credentials:
            if c.credential_name == credential_name:
                matched_cred += 1
                assert_cred2(c.to_credential())
        assert matched_cred == 1
        ...
        # 验证删除
        with pytest.raises(ResourceNotExistError):
            client.get(credential_name=credential_name)
