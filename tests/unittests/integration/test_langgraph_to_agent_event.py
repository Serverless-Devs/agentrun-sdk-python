...
from agentrun.integration.langgraph import AgentRunConverter, to_agui_events
...
def create_tool_message(content: str, tool_call_id: str) -> MagicMock:
...
def convert_and_collect(events: List[Dict]) -> List[Union[str, AgentEvent]]:
    ...
    results = []
    for event in events:
        results.extend(to_agui_events(event))
    return results
...
class TestOnChatModelStreamToolCall:
    ...
    def test_tool_call_subsequent_chunk_with_args(self):
        ...
        results1 = list(
            to_agui_events(first_chunk, tool_call_id_map=tool_call_id_map)
        )
        results2 = list(
            to_agui_events(second_chunk, tool_call_id_map=tool_call_id_map)
        )
...
class TestOnToolStart:
    ...
    def test_tool_start_no_duplicate_if_already_started(self):
        ...
        results = list(
            to_agui_events(event, tool_call_started_set=tool_call_started_set)
        )
