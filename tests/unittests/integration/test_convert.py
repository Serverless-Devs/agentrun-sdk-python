...
class TestConvertWithMockedServer:
    ...
    def test_mock_astream_updates_integration(self):
        ...
        assert "晴天" in results[2].data["result"]

        # 工具调用结束
        assert isinstance(results[3], AgentResult)
        assert results[3].event == EventType.TOOL_CALL_END

        # 最终文本回复
        assert results[4] == "北京今天天气晴朗，气温25度。"
