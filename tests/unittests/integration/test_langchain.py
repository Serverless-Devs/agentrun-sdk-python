"""LangChain Integration 测试

测试 LangChain 框架与 AgentRun 的集成：
- 简单对话（无工具调用）
- 单次工具调用
- 多工具同时调用
- 流式/非流式模式
...
class LangChainTestMixin(IntegrationTestBase):
    """LangChain 测试混入类
    ...
    def create_agent(
        ...
        langchain_tools = list(tools.to_langchain()) if tools else []
...
class TestLangChainIntegration(LangChainTestMixin):
    ...
    def test_stream_options_in_requests(
        self,
        mock_server: MockLLMServer,
        mocked_model: CommonModel,
        mocked_toolset: TestToolSet,
    ...
    def test_stream_options_validation(
        ...
        for req in mock_server.captured_requests:
            if req.stream is True:
                ...
                # LangChain 通过 stream_usage 参数控制，可能不直接传递 stream_options
                # 这里只检查不报错
