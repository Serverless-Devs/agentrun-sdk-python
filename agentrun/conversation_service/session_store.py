...
class SessionStore:
    ...
    def init_tables(self) -> None:
        """创建所有 OTS 表、二级索引和多元索引（同步）。

        包括建表和创建搜索索引，无需再单独调用 init_search_index()。
        """
        self._backend.init_tables()
    ...
    def list_sessions(
        ...
        """列出用户的 Session（按 updated_at 倒序）（同步）。
    ...
    def search_sessions(
        ...
        *,
        ...
        Returns:
            (结果列表, 总匹配数) 二元组。
        """
        return self._backend.search_sessions(
            agent_id,
            user_id=user_id,
            summary_keyword=summary_keyword,
            labels=labels,
    ...
    def delete_events(
        ...
        """只删除 Session 下所有 Event，不删 Session 本身（同步）。
        ...
        Returns:
            删除的事件条数。
        """
        deleted = self._backend.delete_events_by_session(
            agent_id, user_id, session_id
        )
        logger.debug(
            "Deleted %d events for session %s/%s/%s",
    ...
    def delete_session(
        ...
        """
        # 1. 删除所有 Event
        deleted_events = self._backend.delete_events_by_session(
            agent_id, user_id, session_id
        )
        logger.debug(
            "Deleted %d events for session %s/%s/%s",
        ...
        # 2. 删除 Session 级 State
        self._backend.delete_state_row(
            StateScope.SESSION,
            agent_id,
            user_id,
            session_id,
    ...
    def append_event(
        ...
        """追加事件，同时更新 Session 的 updated_at（同步）。
        ...
        # 1. 写入 Event
        seq_id = self._backend.put_event(
            agent_id,
            user_id,
            session_id,
            event_type,
    ...
    def get_recent_events(
        ...
        Returns:
            按 seq_id 正序排列的最近 N 条事件。
        """
        events = self._backend.get_events(
            agent_id,
            user_id,
            session_id,
            direction="BACKWARD",
    ...
    def update_app_state(
        ...
        浅合并语义：top-level key 覆盖，值为 None 表示删除该 key。
        """
        self._apply_delta(StateScope.APP, agent_id, "", "", delta)
    ...
    def get_merged_state(
        ...
        """三级状态浅合并：app_state <- user_state <- session_state（同步）。
    ...
    def from_memory_collection(
        ...
        *,
        ...
        """通过 MemoryCollection 名称创建 SessionStore（同步）。
